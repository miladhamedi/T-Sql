...
Customers از جدول City محاسبه تکرار تعداد کاراکتر الف در فیلد
...
-- Customers از جدول City محاسبه تکرار تعداد کاراکتر غیر الف در فیلد
*/
SELECT 
	City,  LEN(REPLACE(City,N'ا',''))
FROM dbo.Customers;
GO
--------------------------------------------------------------------
...
SELECT 
	...
	CONCAT(State, '*' , Region, '*' , City) COLLATE Persian_100_CI_AI AS Cust_location
FROM dbo.Customers;
GO
...
SELECT
	CustomerID, State, Region, City,
	ISNULL(State,'') + '*' + ISNULL(Region,'') + '*' + ISNULL(City,'') COLLATE Persian_100_CI_AI AS Cust_location
FROM dbo.Customers;
GO